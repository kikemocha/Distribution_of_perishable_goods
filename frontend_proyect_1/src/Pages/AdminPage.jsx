...
function AdminPage() {
  ...
  const editVehicle = async () => {
    try {
      await axios.put(
        `http://127.0.0.1:8000/api/vehicles/${selectedVehicle.id}/`,
        formData
      );
      fetchVehicles();
  ...
  return (
    <div>
      <div className="h-20 flex items-center justify-center sticky top-0 z-10 bg-neutral-100">
        <img
          src="https://www.mercadona.es/static/media/mercadona-logo.a65a34320ed4e902bf30.svg"
          alt="Mercadona Logo"
        />
      </div>
      ...
      <div className="border-t-2 mt-12">
        ...
        <button
          ...
          className="bg-green-600 hover:bg-green-800 text-white px-4 py-2 rounded-lg block mx-auto"
        >
          +
        </button>
        <div className="grid grid-cols-4 px-96 mt-4">
          {vehicles.map((vehicle) => (
            <div
              key={vehicle.id}
              className="relative bg-green-100 hover:bg-green-300 m-2 border-slate-950 h-72 rounded-2xl p-4"
            >
              <button
              ...
              <button
                ...
                className="bg-blue-500 text-white px-3 py-1 rounded-lg block mx-auto mt-2"
              >
                Editar
              </button>
            </div>
	  ...
	  <div className="border-t-2 mt-12">
        ...
        {/* Contenedor de 2 columnas al 50% cada una */}
        <div className="grid grid-cols-2 gap-4 px-4 h-[600px]">
          {/* Columna 1 (lista de órdenes) */}
          <ul className="grid grid-cols-3 gap-2 overflow-auto p-2 bg-green-100 rounded-lg">
            {order.map((order) => (
              <li
                key={order.id}
                className="bg-green-200 rounded-lg p-2 flex flex-col items-center text-xs shadow"
              >
                {/* Icono */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                ...
                {/* Datos */}
                <h5 className="font-bold text-center">{order.cliente_name}</h5>
                <p>{order.mes_anio}</p>
          ...
          {/* Columna 2 (Mapa) */}
          <div className="bg-blue-200 rounded-lg p-4 flex flex-col h-full">
            <h4 className="font-bold text-base mb-2">Mapa OpenLayers</h4>
            
            {/* Contenedor que llena el espacio disponible sin sobresalir */}
