...
df_orders = pd.read_csv(os.path.join(script_dir, 'df_order.csv'))
df_vehicle = pd.read_csv(os.path.join(script_dir, 'df_vehicle.csv'))
df_distance_km = pd.read_excel(os.path.join(script_dir, 'matriz_distancias.xlsx'), index_col=0)
...
clientes = [c for c in df_orders['cliente'].tolist() if c in df_distance_km.columns]
vehiculos = list(df_vehicle["vehiculo_id"])

# Diccionarios clave
demanda = {c: demanda for c, demanda in zip(df_orders['cliente'].tolist(), df_orders["order_demand"].tolist()) if c in df_distance_km.columns}
...
df_distance_km.index = df_distance_km.columns
