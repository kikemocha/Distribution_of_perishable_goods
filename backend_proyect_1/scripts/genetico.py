...
matrix_km = pd.read_excel(os.path.join(script_dir, "matriz_distancias.xlsx"),index_col=0)
...
def algoritmo_genetico():
    ...
    for _, row in pd.read_csv(os.path.join(script_dir, "df_vehicle.csv")).iterrows():
    ...
    for _, row in pd.read_csv(os.path.join(script_dir, "df_order.csv")).iterrows():
        cliente = row['cliente']
